...
project uses. Four author-commissioned AI referee reports (two Claude, two Codex
reviewers; independent of one another) and the author's responses, which
produced major revisions to an earlier draft, are archived in the
repository.}}
\date{July 11, 2026}
...
and this structure overturns the toy-model intuitions the exercise began
with.
...
Simulation exercises in this tradition (including my own earlier model,
preserved in the repository's history) typically invent the key quantity --- the distribution of what
voters stand to gain or lose --- and the invented distributions turn out to
drive the conclusions. Here the inputs are measured and the behavioral layers are explicitly
...
sampling noise. Any reported ``accuracy threshold for elections to work''
...
and across policies; household-correlated error is not simulated here and could change the
levy bloc's effective behavior.
